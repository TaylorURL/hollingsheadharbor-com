...
const CONTACT_ITEMS = [
  {
    ...
    text: '(615) 355-1028',
    textClassName: 'font-medium text-gray-900 group-hover:text-blue-800 transition-colors',
  },
  {
    ...
    text: 'info@smyrnareadymix.com',
    textClassName: 'group-hover:text-blue-800 transition-colors',
  },
  {
    iconPaths: [
...
function ContactIcon({ paths, interactive = true }) {
  return (
    <div
      className={`w-10 h-10 bg-blue-800/10 rounded-xl flex items-center justify-center flex-shrink-0 ${interactive ? 'group-hover:bg-blue-800 transition-colors' : 'mt-0.5'}`}
    >
      <svg
        className={`w-5 h-5 text-blue-800 ${interactive ? 'group-hover:text-white transition-colors' : ''}`}
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
          ...
          <div className="py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            ...
            <div>
              ...
              <ul className="space-y-4 text-gray-600 text-sm">
                {CONTACT_ITEMS.map(({ href, iconPaths, text, textClassName }) => {
                  const isLink = Boolean(href);
                  const content = (
                    <>
                      <ContactIcon paths={iconPaths} interactive={isLink} />
                      <span className={textClassName}>{text}</span>
                    </>
                  );

                  return (
                    <li key={typeof text === 'string' ? text : 'address'}>
                      {isLink ? (
