...
import { SALES_REP_URL, PHONE_NUMBER, PHONE_DISPLAY, SOCIAL_LINKS } from '../constants/urls';

const CONTACT_EMAIL = 'info@smyrnareadymix.com';
const OFFICE_ADDRESS_LINE_1 = '1000 Hollingshead Circle';
const OFFICE_ADDRESS_LINE_2 = 'Murfreesboro, TN 37129';
...
const SOCIAL_ICON_BASE_CLASSES =
...
function SocialLink({ href, label, iconPath }) {
  return (
    <a
      href={href}
      target="_blank"
      ...
      aria-label={label}
      className={SOCIAL_ICON_BASE_CLASSES}
    >
  ...
  return (
    <div>
      ...
      <ul className="space-y-4 text-gray-600 text-sm">
        ...
        <li>
          <a href={`mailto:${CONTACT_EMAIL}`} className="flex items-center space-x-3 group">
            <div className={CONTACT_ICON_CLASSES}>
              <svg
                className="w-5 h-5 text-blue-800 group-hover:text-white transition-colors"
                fill="none"
            ...
            <span className="group-hover:text-blue-800 transition-colors">{CONTACT_EMAIL}</span>
          </a>
        </li>
