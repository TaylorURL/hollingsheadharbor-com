...
function GoogleMap({ locations, selectedLocation, onMarkerClick }) {
  ...
  if (mapError) {
    return (
      <div className="w-full h-full rounded-2xl bg-gray-100 flex flex-col items-center justify-center p-8 text-center" style={{ minHeight: '400px' }}>
        ...
        <p className="text-gray-500 text-sm max-w-xs mb-4">
          View our {locations.length} harbor locations across {[...new Set(locations.map(l => l.state))].length} states below.
        </p>
        <div className="grid grid-cols-3 gap-3 w-full max-w-sm">
          {[...new Set(locations.map(l => l.state))].sort().map(state => (
            <div key={state} className="bg-white rounded-xl p-3 text-center">
              ...
              <div className="text-gray-500 text-xs">{locations.filter(l => l.state === state).length} locations</div>
            </div>
          ))}
        </div>
      </div>
